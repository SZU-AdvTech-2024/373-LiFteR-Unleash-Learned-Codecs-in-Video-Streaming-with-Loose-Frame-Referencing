...
class ME_Spynet(nn.Module):
    ...
    def forward(self, im1, im2):
        ...
        # print("im2list[self.L - 1]",im2list[self.L - 1],"shape_fine ",shape_fine)
        # shape_fine  torch.Size([1, 3, 32, 32])
        ...
        for intLevel in range(self.L):
            ...
            img1,img2,flow=im1list[self.L - 1 - intLevel], flow_warp(im2list[self.L - 1 - intLevel], flowfiledsUpsample), flowfiledsUpsample
        ...
        # print("flowfileds",flowfileds.size())
        return flowfileds


class Tree_Spynet(nn.Module):
    '''
    Modified Spynet to handle parallel processing based on tree dependencies,
    with independent image pyramids for each node.
    '''
    def __init__(self, layername='motion_estimation'):
        super(Tree_Spynet, self).__init__()
        self.L = 4  # Number of pyramid levels
        self.moduleBasic = nn.ModuleList([
            MEBasic(f"{layername}modelL{intLevel + 1}") for intLevel in range(self.L)
        ])

    def build_pyramids(self, img):
        '''
        Build image pyramid for a single image.
        '''
        pyramids = [img]
        for _ in range(1, self.L):
            pyramids.append(F.avg_pool2d(pyramids[-1], kernel_size=2, stride=2))
        return pyramids

    def forward(self, imglist, tree_levels):
        '''
        imglist: list of images in the batch, index corresponds to node.val
        tree_levels: list of lists, each sublist contains TreeNode objects at that level
        '''

        device = imglist[0].device
        # Step 1: Build image pyramids for all nodes
        im1_pyramids = {}
        im2_pyramids = {}
        subGOP_nodes=[]
        for level in tree_levels:
            for node in level:
                im1 = imglist[node.val]
                im2 = imglist[node.parent.val] if node.parent is not None else im1
                im1_pyramids[node.val] = self.build_pyramids(im1)
                im2_pyramids[node.val] = self.build_pyramids(im2)
                if node.val==0:
                    continue
                subGOP_nodes.append(node)
        subGOP_nodes.sort(key=lambda node: node.val)

        _, _, H, W = im1_pyramids[0][-1].size()

        flows = []
        for i in range(len(subGOP_nodes)):
            flows.append(torch.zeros([ 2, H, W], dtype=torch.float32, device=device))
        flows=torch.stack(flows,dim=0)

        for intLevel in range(self.L):
            # Upsample flows except for the coarsest level
            if intLevel > 0:
                flows = F.interpolate(flows, scale_factor=2, mode='bilinear', align_corners=True) * 2.0

            # Collect current pyramid images for all nodes at this pyramid level
            # Note: In pyramids, level 0 is finest, so we index as self.L - 1 - intLevel
            current_im1 = torch.stack([im1_pyramids[node.val][self.L - 1 - intLevel].squeeze(0) for node in subGOP_nodes], dim=0)
            current_im2 = torch.stack([im2_pyramids[node.val][self.L - 1 - intLevel].squeeze(0) for node in subGOP_nodes], dim=0)

            # Warp im2 with current flows
            warped_im2 =flow_warp(current_im2, flows)

            # Concatenate inputs: [im1, warped_im2, flow]
            input_tensor = torch.cat([current_im1, warped_im2, flows], dim=1)  # Shape: [num_nodes, C*2 + 2, H, W]

            # Compute flow updates in batch
            flow_update = self.moduleBasic[intLevel](input_tensor)  # Shape: [num_nodes, 2, H, W]

            # Update flows
            flows = flows + flow_update
